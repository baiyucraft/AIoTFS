<template>
  ...
  <su-popup :show="authType !== ''" round="10" :showClose="true" @close="closeAuthModal">
    <view class="login-wrap">
      ...
      >
        <!-- 7.1 微信小程序的快捷登录 -->
        <view v-if="yu.$platform.name === 'WechatMiniProgram'" class="ss-flex register-box">
        ...
        <button
          v-if="
            ['WechatOfficialAccount', 'WechatMiniProgram', 'App'].includes(yu.$platform.name) &&
              yu.$platform.isWechatInstalled
          "
        ...
        >
          <image
            ...
            :src="yu.$url.static('/static/img/shop/platform/wechat.png')"
          />
        </button>

        <!-- 7.3 iOS 登录 TODO 芋艿：等后面搞 App 再弄 -->
        <button
          v-if="yu.$platform.os === 'ios' && yu.$platform.name === 'App'"
          @tap="thirdLogin('apple')"
          class="ss-reset-button auto-login-btn"
        >
          <image
            class="auto-login-img"
            :src="yu.$url.static('/static/img/shop/platform/apple.png')"
          />
        </button>
      </view>
...
import { closeAuthModal, showAuthModal } from '@yu/hooks/useModal'

// const app = yu.$store.useAppStore()
// const appInfo = computed(() => app.info)

const modalStore = yu.$store.useModalStore()
// 授权弹窗类型
...
function onProtocol(title) {
  closeAuthModal()
  yu.$router.go('/pages/public/richtext', {
    title
  })
}
...
const thirdLogin = async(provider) => {
  if (!state.protocol) {
    ...
    yu.$helper.toast('请勾选同意')
    return
  }
  const loginRes = await yu.$platform.useProvider(provider).login()
  if (loginRes) {
    closeAuthModal()
    // 触发小程序授权信息弹框
    // #ifdef MP-WEIXIN
...
const getPhoneNumber = async(e) => {
  if (e.detail.errMsg !== 'getPhoneNumber:ok') {
    yu.$helper.toast('快捷登录失败')
    return
  }
  const result = await yu.$platform.useProvider().mobileLogin(e.detail)
  if (result) {
    closeAuthModal()
  }
}
