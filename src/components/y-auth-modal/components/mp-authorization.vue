    ...
    >
      ...
      <uni-forms-item name="avatar" label="头像">
        ...
        >
          <image
            class="avatar-img"
            :src="yu.$url.cdn(state.model.avatar)"
            mode="aspectFill"
            @tap="yu.$router.go('/pages/user/info')"
          />
...
import yu from '@yu'
import { closeAuthModal } from '@yu/hooks/useModal'
...
async function onConfirm() {
  ...
  if (!nickname) {
    yu.$helper.toast('请输入昵称')
    return
  }
  if (!avatar) {
    yu.$helper.toast('请选择头像')
    return
  }
  // 发起更新
