...
<template>
  <view>
    ...
    <!-- 微信独有：读取手机号 -->
    <button
      v-if="'WechatMiniProgram' === yu.$platform.name"
...
import yu from '@yu'
import { code, mobile } from '@yu/validate/form'
import { closeAuthModal, getSmsCode, getSmsTimer } from '@yu/hooks/useModal'
...
async function getPhoneNumber(e) {
  ...
  const result = await yu.$platform.useProvider().bindUserPhoneNumber(e.detail)
  if (result) {
    user.getInfo()
    closeAuthModal()
  }
