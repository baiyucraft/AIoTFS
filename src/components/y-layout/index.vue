<template>
  <view
    class="page-app"
    :class="['theme-' + sys.state.mode, 'main-' + sys.state.theme, 'font-' + sys.state.fontSize]"
  >
    <view class="page-main" :style="[bgMain]">
      ...
      <view class="page-body" :style="[bgBody]">
        ...
        <!-- 底部导航 -->
        <y-tabbar v-if="tabbar !== ''" :path="tabbar" />
      </view>
    </view>
    ...
    <view class="page-modal">
      ...
      <y-auth-modal />
      <!-- 全局分享弹窗 -->
      <y-share-modal :shareInfo="shareInfo" />
      <!-- 全局快捷入口 -->
      <y-menu-tools />
    </view>
  </view>
...
const sysStore = yu.$store.useSysStore()
...
const bgMain = computed(() => {
  if (navbarMode.value === 'inner') {
    return {
      background: `${props.bgStyle.backgroundColor} url(${yu.$url.cdn(
...
const shareInfo = computed(() => {
  if (props.onShareAppMessage === true) {
    ...
    if (!isEmpty(props.onShareAppMessage)) {
      yu.$platform.share.updateShareInfo(props.onShareAppMessage)
...
onShow(() => {
  if (!isEmpty(shareInfo.value)) {
    yu.$platform.share.updateShareInfo(shareInfo.value)
  }
})
</script>
