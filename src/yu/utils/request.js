...
import { baseUrl, apiPath } from '@yu/config'
...
http.interceptors.response.use(
  (response) => {
    ...
    // TODO H：自定义
    if (response.config.url.indexOf('/predictModel') >= 0) {
...
const request = (config) => {
  if (config.url.indexOf('/app/test/') >= 0 || config.url.indexOf('/predictModel') >= 0) {
    config.url = 'http://192.168.1.164:1111' + config.url
  }
