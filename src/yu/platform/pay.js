// #ifdef H5
import $wxsdk from '@yu/libs/sdk-h5-weixin'
// #endif
import $platform from '.'
import $router from '@yu/router'
import $helper, { getRootUrl } from '@yu/helper'
// TODO H: API
...
export default class SheepPay {
  ...
  prepay(channel) {
    ...
    if (['wx_pub', 'wx_lite'].includes(channel)) {
      return $platform.useProvider('wechat').getOpenid().then((openid) => {
    ...
    PayOrderApi.submitOrder(data).then((res) => {
      res.code === 0 && Promise.resolve(res)
      // 失败时
    ...
    $wxsdk.wxpay(payConfig, {
      ...
      fail: (error) => {
        if (error.errMsg.indexOf('chooseWXPay:没有此SDK或暂不支持此SDK模拟') >= 0) {
          $helper.toast('发起微信支付失败，原因：可能是微信开发者工具不支持，建议使用微信打开网页后支付')
          return
        }
        this.payResult('fail')
      }
